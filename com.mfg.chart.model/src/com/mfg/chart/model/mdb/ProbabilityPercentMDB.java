...
/**
 * <p>
 * This class provides the API to manipulate ProbabilityPercent files. 
 * Here you will find the methods to modify and query the ProbabilityPercent files. 
 * </p>
 * <p>
 ...
 * </p>
 * <h3>ProbabilityPercent definition</h3>
 * <table border=1>
 *	<caption>ProbabilityPercent</caption>
 *	<tr>
 *		<td>Column</td>
 *		<td>Type</td>
 *		<td>Order</td>
 ...
 *	</tr>
 * </table>
 * <h3>ProbabilityPercentMDB API</h3>
 * <p>
 * Now let's see the operations you can perform using this class on ProbabilityPercent files:
 * </p>
 *
 ...
 * ProbabilityPercentMDB mdb = session.connectTo_ProbabilityPercentMDB("probabilitypercent.mdb");
 * 
 * // request the appender.
 * ProbabilityPercentMDB.Appender app = mdb.appender(); 
 *
 ...
 * </p>
 * <pre>
 * ProbabilityPercentMDB mdb = ...;
 * long start = ...;
 * long stop = ...;
 *
 * // request a sequential cursor from start to stop
 * ProbabilityPercent.Cursor cursor = mdb.cursor(start, stop);
 *
 ...
 * 			+ cursor.posTHPrice + " "
 * 			+ cursor.negTHPrice + " "
 * 			+ cursor.posCurrentPrice + " "
 * 			+ cursor.negCurrentPrice + " "
 * 			+ cursor.time + " "
 * 			+ cursor.posTradeDirection + " "
 * 		);
 * }
 ...
 * ProbabilityPercentMDB.RandomCursor cursor = mdb.randomCursor();
 *
 ...
 *
 * // the new values 										
 * long new_val_posTHPrice = ...;
 * long new_val_negTHPrice = ...;
 * long new_val_posCurrentPrice = ...;
 * long new_val_negCurrentPrice = ...;
 * long new_val_time = ...;
 * boolean new_val_posTradeDirection = ...;
 *
 * mdb.replace(index 
 * 		, new_val_posTHPrice
 * 		, new_val_negTHPrice
 * 		, new_val_posCurrentPrice
 * 		, new_val_negCurrentPrice
 * 		, new_val_time
 * 		, new_val_posTradeDirection
 *		);
 * </pre>
 * <p>
 ...
 * </p>
 * <ul>
 * <li>{@link ProbabilityPercentMDB#replace_posTHPrice(long, long)}: To replace the posTHPrice value.</li>
 * <li>{@link ProbabilityPercentMDB#replace_negTHPrice(long, long)}: To replace the negTHPrice value.</li>
 * <li>{@link ProbabilityPercentMDB#replace_posCurrentPrice(long, long)}: To replace the posCurrentPrice value.</li>
 * <li>{@link ProbabilityPercentMDB#replace_negCurrentPrice(long, long)}: To replace the negCurrentPrice value.</li>
 * <li>{@link ProbabilityPercentMDB#replace_time(long, long)}: To replace the time value.</li>
 * <li>{@link ProbabilityPercentMDB#replace_posTradeDirection(long, boolean)}: To replace the posTradeDirection value.</li>
 * </ul>
 *
 * <h3>List API</h3>
 * TODO: Documentation is comming
 *
 * @see BaseChartMDBSession#connectTo_ProbabilityPercentMDB(String)
 */

public final class ProbabilityPercentMDB
/* BEGIN MDB EXTENDS */
		extends MDB<ProbabilityPercentMDB.Record>
/* END MDB EXTENDS */
{
...
/* END USER MDB */
	/**
	 * ProbabilityPercent's meta-data: column names.
	 */
	...
	/**
	 * ProbabilityPercent's meta-data: column Java types.
	 */
	...
	/**
	 * ProbabilityPercent's meta-data: column Java types size (in bytes).
	 */
	...
	/**
	 * ProbabilityPercent's meta-data: virtual column flags.
	 */
	...
	/**
	 * ProbabilityPercent's meta-data: column byte-offset.
	 */
	...
	/**
	 * ProbabilityPercent's meta-data: size of the record, in bytes.
	 */
	public static final int RECORD_SIZE = 41;
	
	/**
	* posTHPrice's meta-data: index in a record.
	*/	
	public static final int COLUMN_POSTHPRICE = 0;
	/**
	* negTHPrice's meta-data: index in a record.
	*/	
	public static final int COLUMN_NEGTHPRICE = 1;
	/**
	* posCurrentPrice's meta-data: index in a record.
	*/	
	public static final int COLUMN_POSCURRENTPRICE = 2;
	/**
	* negCurrentPrice's meta-data: index in a record.
	*/	
	public static final int COLUMN_NEGCURRENTPRICE = 3;
	/**
	* time's meta-data: index in a record.
	*/	
	public static final int COLUMN_TIME = 4;
	/**
	* posTradeDirection's meta-data: index in a record.
	*/	
	public static final int COLUMN_POSTRADEDIRECTION = 5;

	/**
	 * ProbabilityPercent's meta-data: UUID used in schemas.
	 */
	public static final String TABLE_ID = "1f92e2bc-e710-4953-a811-4e60c1eab897";
	
	/**
	 * ProbabilityPercent's meta-data: signature used to check schema changes.
	 */ 
	public static final String TABLE_SIGNATURE = "05aa762b-6c4b-4839-bd2a-55fa734e3dca LONG; daffad05-ac75-459c-b4fb-32bb2df5f638 LONG; 2fcff986-02a0-4ae6-9e3d-f85399dd8123 LONG; 3ead80b8-2c1b-4ca9-992f-0c308c57620e LONG; f2c38869-4631-4461-b742-ba429823efe7 LONG; 79ed25ff-3063-41a4-b758-f60127c2471a BOOLEAN; ";


	private Appender _appender;
	private ByteBuffer _replaceBuffer; 
	private ByteBuffer _replaceBuffer_posTHPrice;
	private ByteBuffer _replaceBuffer_negTHPrice;
	private ByteBuffer _replaceBuffer_posCurrentPrice;
	private ByteBuffer _replaceBuffer_negCurrentPrice;
	private ByteBuffer _replaceBuffer_time;
	private ByteBuffer _replaceBuffer_posTradeDirection;
	int _rbufSize;
	 ...
	 */
	public ProbabilityPercentMDB(BaseChartMDBSession session, File file, int bufferSize, SessionMode mode) throws IOException {
		...
		if (!_memory) {		
			_replaceBuffer = ByteBuffer.allocate(41);		
			_replaceBuffer_posTHPrice = ByteBuffer.allocate(8);
			_replaceBuffer_negTHPrice = ByteBuffer.allocate(8);
			_replaceBuffer_posCurrentPrice = ByteBuffer.allocate(8);
			_replaceBuffer_negCurrentPrice = ByteBuffer.allocate(8);
			_replaceBuffer_time = ByteBuffer.allocate(8);
			_replaceBuffer_posTradeDirection = ByteBuffer.allocate(1);
		}
	}	

	/**
	* ProbabilityPercent record structure.
	*/
...
/* END RECORD EXTENDS */	{
		/**
		* Represents the posTHPrice column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of posTHPrice</caption>
		* <tr><td>Column</td><td>posTHPrice</td></tr>
		* <tr><td>Type</td><td>LONG</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public long posTHPrice; /* 0 */
		/**
		* Represents the negTHPrice column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of negTHPrice</caption>
		* <tr><td>Column</td><td>negTHPrice</td></tr>
		* <tr><td>Type</td><td>LONG</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public long negTHPrice; /* 1 */
		/**
		* Represents the posCurrentPrice column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of posCurrentPrice</caption>
		* <tr><td>Column</td><td>posCurrentPrice</td></tr>
		* <tr><td>Type</td><td>LONG</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public long posCurrentPrice; /* 2 */
		/**
		* Represents the negCurrentPrice column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of negCurrentPrice</caption>
		* <tr><td>Column</td><td>negCurrentPrice</td></tr>
		* <tr><td>Type</td><td>LONG</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public long negCurrentPrice; /* 3 */
		/**
		...
		*/
		public long time; /* 4 */
		/**
		* Represents the posTradeDirection column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of posTradeDirection</caption>
		* <tr><td>Column</td><td>posTradeDirection</td></tr>
		* <tr><td>Type</td><td>BOOLEAN</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public boolean posTradeDirection; /* 5 */
		...
		*/
		...
		public String toString() {
			return "ProbabilityPercent [ "
				 + "posTHPrice=" + posTHPrice + " "	
				 + "negTHPrice=" + negTHPrice + " "	
				 + "posCurrentPrice=" + posCurrentPrice + " "	
				 + "negCurrentPrice=" + negCurrentPrice + " "	
				 + "time=" + time + " "	
				 + "posTradeDirection=" + posTradeDirection + " "	
				 + " ]";
		}
		...
		*/
		...
		public Object[] toArray() {
			return new Object[] {
							Long.valueOf(posTHPrice),
							Long.valueOf(negTHPrice),
							Long.valueOf(posCurrentPrice),
							Long.valueOf(negCurrentPrice),
							Long.valueOf(time),
							Boolean.valueOf(posTradeDirection),
			 			};
		}
		...
		*/
		...
		public Object get(int columnIndex) {
			switch(columnIndex) {
				case 0: return posTHPrice;
				case 1: return negTHPrice;
				case 2: return posCurrentPrice;
				case 3: return negCurrentPrice;
				case 4: return time;
				case 5: return posTradeDirection;
		...
		*/ 
		public void update(Record record) {
			this.posTHPrice = record.posTHPrice;
			this.negTHPrice = record.negTHPrice;
			this.posCurrentPrice = record.posCurrentPrice;
			this.negCurrentPrice = record.negCurrentPrice;
			this.time = record.time;
			this.posTradeDirection = record.posTradeDirection;
		}
...
/**
	...
	* while( ... ) {
	* 	ap.posTHPrice = getPosTHPrice();	
	* 	ap.negTHPrice = getNegTHPrice();	
	* 	ap.posCurrentPrice = getPosCurrentPrice();	
	* 	ap.negCurrentPrice = getNegCurrentPrice();	
	* 	ap.time = getTime();	
	* 	ap.posTradeDirection = getPosTradeDirection();	
	* 	ap.append();
	* }
	* ap.close();
	* </pre>
	*/
	public final class Appender implements IAppender<Record> {
		...
		protected ProbabilityPercentMDB _mdb;	
		...
		*/
		Appender() throws IOException {
			_mdb = ProbabilityPercentMDB.this;
			if (!_memory) {
				_buf = ByteBuffer.allocate(_bufferSize * 41);
				reconnectFile();
			}
		}
		...
		*/
		...
		public void append() throws IOException {
			if (_basic) {
				try {
					...
					_buf.putLong(this.posTHPrice);
					_buf.putLong(this.negTHPrice);
					_buf.putLong(this.posCurrentPrice);
					_buf.putLong(this.negCurrentPrice);
					_buf.putLong(this.time);
					_buf.put((byte) (this.posTradeDirection? 1 : 0));
				...
				try {
					...
					r.posTHPrice = this.posTHPrice;
					r.negTHPrice = this.negTHPrice;
					r.posCurrentPrice = this.posCurrentPrice;
					r.negCurrentPrice = this.negCurrentPrice;
					r.time = this.time;
					r.posTradeDirection = this.posTradeDirection;
					_rbufSize++;
					_size++;
				} catch (Exception e) {
		...
		public void append(Record record) throws IOException {	
			...
			try {										
				...
				Record r = _rbuf[_rbufSize];
				r.posTHPrice = record.posTHPrice;
				r.negTHPrice = record.negTHPrice;
				r.posCurrentPrice = record.posCurrentPrice;
				r.negCurrentPrice = record.negCurrentPrice;
				r.time = record.time;
				r.posTradeDirection = record.posTradeDirection;
				_rbufSize++;
				_size++;
			} catch (Exception e) {
		...
		*/
		public void flush() throws IOException {	
			...
			for(int j = 0; j < _rbufSize; j++) {						
				Record r = _rbuf[j];
				_buf.putLong(r.posTHPrice);
				_buf.putLong(r.negTHPrice);
				_buf.putLong(r.posCurrentPrice);
				_buf.putLong(r.negCurrentPrice);
				_buf.putLong(r.time);
				_buf.put((byte) (r.posTradeDirection? 1 : 0));
			}
			_buf.rewind();
			_buf.limit(_rbufSize * 41);
			_channel.write(_buf);
			_buf.limit(_buf.capacity());
			_buf.rewind();
		...
		*/
		@Override
		public ProbabilityPercentMDB getMDB() {
			return _mdb;
		}
	...
	*/
	...
	public final class Cursor implements ISeqCursor<Record> {
		...
		*/
		Cursor(RandomAccessFile raf, FileChannel channel, long start, long stop, int bufferSize) throws IOException {
			...
			if (!_memory) {
				_raf = raf;
				_channel = channel;
				_channel.position(start * 41);
				_buffer = ByteBuffer.allocate(bufferSize * 41);
		...
		*/
		...
		public synchronized boolean next() throws IOException {
			...
			if (!_basic) {
				...
				try {
					if (_rbufSize > 0 && _row >= _rbufPos) {
						...
						this.posTHPrice = r.posTHPrice;
						this.negTHPrice = r.negTHPrice;
						this.posCurrentPrice = r.posCurrentPrice;
						this.negCurrentPrice = r.negCurrentPrice;
						this.time = r.time;
						this.posTradeDirection = r.posTradeDirection;
						_row ++;
						return true;	
					}
				} finally {
			...
			if (!_memory) {
				...
				this.posTHPrice = _buffer.getLong();
				this.negTHPrice = _buffer.getLong();
				this.posCurrentPrice = _buffer.getLong();
				this.negCurrentPrice = _buffer.getLong();
				this.time = _buffer.getLong();
				this.posTradeDirection = _buffer.get() == 0? false : true;
				_row ++;
			}
			return true;
		}
		...
		*/
		@Override
		public ProbabilityPercentMDB getMDB() {
			return ProbabilityPercentMDB.this;
		}
	}
		...
		public final class RandomCursor implements IRandomCursor<Record> {
			...
			RandomCursor() throws IOException {
				...
				if (!_memory) {
					...
					_channel = _raf.getChannel();
					_buffer = ByteBuffer.allocate(41);
			...
			public synchronized void seek(long position) throws IOException {
				...
				_row = position;
				_buffer.rewind();
				_channel.read(_buffer, position * 41);
				_buffer.rewind();
				this.posTHPrice = _buffer.getLong();
				this.negTHPrice = _buffer.getLong();
				this.posCurrentPrice = _buffer.getLong();
	...
	*/
	...
	public static void writeBuffer(Cursor obj, ByteBuffer buffer) {
		buffer.putLong(obj.posTHPrice); 
		buffer.putLong(obj.negTHPrice); 
		buffer.putLong(obj.posCurrentPrice); 
		buffer.putLong(obj.negCurrentPrice); 
		buffer.putLong(obj.time); 
		buffer.put(obj.posTradeDirection ? (byte) 1 : (byte) 0); 
	}
	 ...
	 * @param val_posTHPrice The value for column posTHPrice.
	 * @param val_negTHPrice The value for column negTHPrice.
	 * @param val_posCurrentPrice The value for column posCurrentPrice.
	 * @param val_negCurrentPrice The value for column negCurrentPrice.
	 * @param val_time The value for column time.
	 * @param val_posTradeDirection The value for column posTradeDirection.
	* @throws IOException If there is any I/O error.
	 */
	public void replace(long index 
							, long val_posTHPrice
							, long val_negTHPrice
							, long val_posCurrentPrice
							, long val_negCurrentPrice
							, long val_time
							, boolean val_posTradeDirection
			) throws IOException {
		...
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					...
					if (pos < _rbufSize) {
						Record r = _rbuf[pos];
						r.posTHPrice = val_posTHPrice;
						r.negTHPrice = val_negTHPrice;
						r.posCurrentPrice = val_posCurrentPrice;
						r.negCurrentPrice = val_negCurrentPrice;
						r.time = val_time;
						r.posTradeDirection = val_posTradeDirection;
					}
					return;
				} 				
			}
			...
			_replaceBuffer.putLong(val_posTHPrice);
			_replaceBuffer.putLong(val_negTHPrice);
			_replaceBuffer.putLong(val_posCurrentPrice);
			_replaceBuffer.putLong(val_negCurrentPrice);
			_replaceBuffer.putLong(val_time);
			_replaceBuffer.put((byte) (val_posTradeDirection ? 1 : 0));
			_replaceBuffer.rewind();
			appender();
			_appender._channel.write(_replaceBuffer, index * 41);
		} finally {
	 ...
	 * Also you can use {@link #replace(long , long, long, long, long, long, boolean)}.
	 *
	 * @param index The index to update.
	 * @param record Contains the data to set.
	 * @see #replace(long , long, long, long, long, long, boolean)
	 * @throws IOException If there is any I/O error.
	 */
	public void replace(long index, Record record) throws IOException {
		replace(index 
					, record.posTHPrice		
					, record.negTHPrice		
					, record.posCurrentPrice		
					, record.negCurrentPrice		
					, record.time		
					, record.posTradeDirection		
				);			
	}

	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "posTHPrice" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , long, long, long, long, long, boolean)},
	 ...
	 * @param value The new value to set to the column "posTHPrice".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_posTHPrice(long index, long value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].posTHPrice = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_posTHPrice.rewind();
			_replaceBuffer_posTHPrice.putLong(value);
			_replaceBuffer_posTHPrice.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_posTHPrice, index * 41 + 0);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "negTHPrice" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , long, long, long, long, long, boolean)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "negTHPrice".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_negTHPrice(long index, long value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].negTHPrice = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_negTHPrice.rewind();
			_replaceBuffer_negTHPrice.putLong(value);
			_replaceBuffer_negTHPrice.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_negTHPrice, index * 41 + 8);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "posCurrentPrice" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , long, long, long, long, long, boolean)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "posCurrentPrice".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_posCurrentPrice(long index, long value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].posCurrentPrice = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_posCurrentPrice.rewind();
			_replaceBuffer_posCurrentPrice.putLong(value);
			_replaceBuffer_posCurrentPrice.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_posCurrentPrice, index * 41 + 16);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "negCurrentPrice" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , long, long, long, long, long, boolean)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "negCurrentPrice".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_negCurrentPrice(long index, long value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].negCurrentPrice = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_negCurrentPrice.rewind();
			_replaceBuffer_negCurrentPrice.putLong(value);
			_replaceBuffer_negCurrentPrice.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_negCurrentPrice, index * 41 + 24);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	...
	public void replace_time(long index, long value) throws IOException {
		...
		try {
			...
			_replaceBuffer_time.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_time, index * 41 + 32);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "posTradeDirection" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , long, long, long, long, long, boolean)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "posTradeDirection".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_posTradeDirection(long index, boolean value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].posTradeDirection = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_posTradeDirection.rewind();
			_replaceBuffer_posTradeDirection.put((byte) (value ? 1 : 0));
			_replaceBuffer_posTradeDirection.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_posTradeDirection, index * 41 + 40);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	...
	*/
	...
	public long fsize() throws IOException {		
		return _memory? 0 : _file.length() / 41;
	}
