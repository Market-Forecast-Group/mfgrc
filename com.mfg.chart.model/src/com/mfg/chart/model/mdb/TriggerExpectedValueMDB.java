...
/**
 * <p>
 * This class provides the API to manipulate TriggerExpectedValue files. 
 * Here you will find the methods to modify and query the TriggerExpectedValue files. 
 * </p>
 * <p>
 ...
 * </p>
 * <h3>TriggerExpectedValue definition</h3>
 * <table border=1>
 *	<caption>TriggerExpectedValue</caption>
 *	<tr>
 *		<td>Column</td>
 *		<td>Type</td>
 *		<td>Order</td>
 ...
 *	</tr>
 * </table>
 * <h3>TriggerExpectedValueMDB API</h3>
 * <p>
 * Now let's see the operations you can perform using this class on TriggerExpectedValue files:
 * </p>
 *
 ...
 * TriggerExpectedValueMDB mdb = session.connectTo_TriggerExpectedValueMDB("triggerexpectedvalue.mdb");
 * 
 * // request the appender.
 * TriggerExpectedValueMDB.Appender app = mdb.appender(); 
 *
 * // set the appender values
 * app.negValue = ...;
 * app.posValue = ...;
 * app.time = ...;
 * app.negSwingProjection = ...;
 * app.posSwingProjection = ...;
 ...
 * </p>
 * <pre>
 * TriggerExpectedValueMDB mdb = ...;
 * long start = ...;
 * long stop = ...;
 *
 * // request a sequential cursor from start to stop
 * TriggerExpectedValue.Cursor cursor = mdb.cursor(start, stop);
 *
 ...
 * 			+ cursor.posValue + " "
 * 			+ cursor.time + " "
 * 			+ cursor.negSwingProjection + " "
 * 			+ cursor.posSwingProjection + " "
 * 		);
 * }
 ...
 * TriggerExpectedValueMDB.RandomCursor cursor = mdb.randomCursor();
 *
 ...
 *
 * // the new values 										
 * double new_val_negValue = ...;
 * double new_val_posValue = ...;
 * long new_val_time = ...;
 * double new_val_negSwingProjection = ...;
 * double new_val_posSwingProjection = ...;
 *
 * mdb.replace(index 
 * 		, new_val_negValue
 * 		, new_val_posValue
 * 		, new_val_time
 * 		, new_val_negSwingProjection
 * 		, new_val_posSwingProjection
 *		);
 * </pre>
 * <p>
 ...
 * </p>
 * <ul>
 * <li>{@link TriggerExpectedValueMDB#replace_negValue(long, double)}: To replace the negValue value.</li>
 * <li>{@link TriggerExpectedValueMDB#replace_posValue(long, double)}: To replace the posValue value.</li>
 * <li>{@link TriggerExpectedValueMDB#replace_time(long, long)}: To replace the time value.</li>
 * <li>{@link TriggerExpectedValueMDB#replace_negSwingProjection(long, double)}: To replace the negSwingProjection value.</li>
 * <li>{@link TriggerExpectedValueMDB#replace_posSwingProjection(long, double)}: To replace the posSwingProjection value.</li>
 * </ul>
 *
 * <h3>List API</h3>
 * TODO: Documentation is comming
 *
 * @see BaseChartMDBSession#connectTo_TriggerExpectedValueMDB(String)
 */

public final class TriggerExpectedValueMDB
/* BEGIN MDB EXTENDS */
		extends MDB<TriggerExpectedValueMDB.Record>
/* END MDB EXTENDS */
{
...
/* END USER MDB */
	/**
	 * TriggerExpectedValue's meta-data: column names.
	 */
	...
	/**
	 * TriggerExpectedValue's meta-data: column Java types.
	 */
	...
	/**
	 * TriggerExpectedValue's meta-data: column Java types size (in bytes).
	 */
	...
	/**
	 * TriggerExpectedValue's meta-data: virtual column flags.
	 */
	...
	/**
	 * TriggerExpectedValue's meta-data: column byte-offset.
	 */
	...
	/**
	 * TriggerExpectedValue's meta-data: size of the record, in bytes.
	 */
	public static final int RECORD_SIZE = 40;
	
	/**
	* negValue's meta-data: index in a record.
	*/	
	public static final int COLUMN_NEGVALUE = 0;
	/**
	* posValue's meta-data: index in a record.
	*/	
	public static final int COLUMN_POSVALUE = 1;
	/**
	* time's meta-data: index in a record.
	*/	
	public static final int COLUMN_TIME = 2;
	/**
	* negSwingProjection's meta-data: index in a record.
	*/	
	public static final int COLUMN_NEGSWINGPROJECTION = 3;
	/**
	* posSwingProjection's meta-data: index in a record.
	*/	
	public static final int COLUMN_POSSWINGPROJECTION = 4;

	/**
	 * TriggerExpectedValue's meta-data: UUID used in schemas.
	 */
	public static final String TABLE_ID = "84c11d87-2519-4895-b34a-a5360889f621";
	
	/**
	 * TriggerExpectedValue's meta-data: signature used to check schema changes.
	 */ 
	public static final String TABLE_SIGNATURE = "70b74c6a-5bb1-4725-82a3-4115cf1b6ac4 DOUBLE; 5bb92c9a-5518-4435-b452-686aad92606e DOUBLE; cfb8673d-8ed6-4f20-95b1-4c11bd7b4bb3 LONG; c9a3bc9c-2108-4155-a230-a68dfbed229e DOUBLE; a7ee65e8-22ff-4bbf-870a-182a5c1ace9d DOUBLE; ";


	private Appender _appender;
	private ByteBuffer _replaceBuffer; 
	private ByteBuffer _replaceBuffer_negValue;
	private ByteBuffer _replaceBuffer_posValue;
	private ByteBuffer _replaceBuffer_time;
	private ByteBuffer _replaceBuffer_negSwingProjection;
	private ByteBuffer _replaceBuffer_posSwingProjection;
	int _rbufSize;
	 ...
	 */
	public TriggerExpectedValueMDB(BaseChartMDBSession session, File file, int bufferSize, SessionMode mode) throws IOException {
		...
		if (!_memory) {		
			_replaceBuffer = ByteBuffer.allocate(40);		
			_replaceBuffer_negValue = ByteBuffer.allocate(8);
			_replaceBuffer_posValue = ByteBuffer.allocate(8);
			_replaceBuffer_time = ByteBuffer.allocate(8);
			_replaceBuffer_negSwingProjection = ByteBuffer.allocate(8);
			_replaceBuffer_posSwingProjection = ByteBuffer.allocate(8);
		}
	}	

	/**
	* TriggerExpectedValue record structure.
	*/
...
/* END RECORD EXTENDS */	{
		/**
		* Represents the negValue column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of negValue</caption>
		* <tr><td>Column</td><td>negValue</td></tr>
		* <tr><td>Type</td><td>DOUBLE</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public double negValue; /* 0 */
		/**
		* Represents the posValue column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of posValue</caption>
		* <tr><td>Column</td><td>posValue</td></tr>
		* <tr><td>Type</td><td>DOUBLE</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public double posValue; /* 1 */
		/**
		...
		*/
		public long time; /* 2 */
		/**
		* Represents the negSwingProjection column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of negSwingProjection</caption>
		* <tr><td>Column</td><td>negSwingProjection</td></tr>
		* <tr><td>Type</td><td>DOUBLE</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public double negSwingProjection; /* 3 */
		/**
		* Represents the posSwingProjection column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of posSwingProjection</caption>
		* <tr><td>Column</td><td>posSwingProjection</td></tr>
		* <tr><td>Type</td><td>DOUBLE</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public double posSwingProjection; /* 4 */
		...
		*/
		...
		public String toString() {
			return "TriggerExpectedValue [ "
				 + "negValue=" + negValue + " "	
				 + "posValue=" + posValue + " "	
				 + "time=" + time + " "	
				 + "negSwingProjection=" + negSwingProjection + " "	
				 + "posSwingProjection=" + posSwingProjection + " "	
				 + " ]";
		}
		...
		*/
		...
		public Object[] toArray() {
			return new Object[] {
							Double.valueOf(negValue),
							Double.valueOf(posValue),
							Long.valueOf(time),
							Double.valueOf(negSwingProjection),
							Double.valueOf(posSwingProjection),
			 			};
		}
		...
		*/
		...
		public Object get(int columnIndex) {
			switch(columnIndex) {
				case 0: return negValue;
				case 1: return posValue;
				case 2: return time;
				case 3: return negSwingProjection;
				case 4: return posSwingProjection;
		...
		*/ 
		public void update(Record record) {
			this.negValue = record.negValue;
			this.posValue = record.posValue;
			this.time = record.time;
			this.negSwingProjection = record.negSwingProjection;
			this.posSwingProjection = record.posSwingProjection;
		}
...
/**
	...
	* while( ... ) {
	* 	ap.negValue = getNegValue();	
	* 	ap.posValue = getPosValue();	
	* 	ap.time = getTime();	
	* 	ap.negSwingProjection = getNegSwingProjection();	
	* 	ap.posSwingProjection = getPosSwingProjection();	
	* 	ap.append();
	* }
	* ap.close();
	* </pre>
	*/
	public final class Appender implements IAppender<Record> {
		...
		protected TriggerExpectedValueMDB _mdb;	
		...
		*/
		Appender() throws IOException {
			_mdb = TriggerExpectedValueMDB.this;
			if (!_memory) {
				_buf = ByteBuffer.allocate(_bufferSize * 40);
				reconnectFile();
			}
		}
		...
		*/
		...
		public void append() throws IOException {
			if (_basic) {
				try {
					...
					_buf.putDouble(this.negValue);
					_buf.putDouble(this.posValue);
					_buf.putLong(this.time);
					_buf.putDouble(this.negSwingProjection);
					_buf.putDouble(this.posSwingProjection);
				...
				try {
					...
					r.negValue = this.negValue;
					r.posValue = this.posValue;
					r.time = this.time;
					r.negSwingProjection = this.negSwingProjection;
					r.posSwingProjection = this.posSwingProjection;
					_rbufSize++;
					_size++;
				} catch (Exception e) {
		...
		public void append(Record record) throws IOException {	
			...
			try {										
				...
				Record r = _rbuf[_rbufSize];
				r.negValue = record.negValue;
				r.posValue = record.posValue;
				r.time = record.time;
				r.negSwingProjection = record.negSwingProjection;
				r.posSwingProjection = record.posSwingProjection;
				_rbufSize++;
				_size++;
			} catch (Exception e) {
		...
		*/
		public void flush() throws IOException {	
			...
			for(int j = 0; j < _rbufSize; j++) {						
				Record r = _rbuf[j];
				_buf.putDouble(r.negValue);
				_buf.putDouble(r.posValue);
				_buf.putLong(r.time);
				_buf.putDouble(r.negSwingProjection);
				_buf.putDouble(r.posSwingProjection);
			}
			_buf.rewind();
			_buf.limit(_rbufSize * 40);
			_channel.write(_buf);
			_buf.limit(_buf.capacity());
			_buf.rewind();
		...
		*/
		@Override
		public TriggerExpectedValueMDB getMDB() {
			return _mdb;
		}
	...
	*/
	...
	public final class Cursor implements ISeqCursor<Record> {
		...
		*/
		Cursor(RandomAccessFile raf, FileChannel channel, long start, long stop, int bufferSize) throws IOException {
			...
			if (!_memory) {
				_raf = raf;
				_channel = channel;
				_channel.position(start * 40);
				_buffer = ByteBuffer.allocate(bufferSize * 40);
		...
		*/
		...
		public synchronized boolean next() throws IOException {
			...
			if (!_basic) {
				...
				try {
					if (_rbufSize > 0 && _row >= _rbufPos) {
						...
						this.negValue = r.negValue;
						this.posValue = r.posValue;
						this.time = r.time;
						this.negSwingProjection = r.negSwingProjection;
						this.posSwingProjection = r.posSwingProjection;
						_row ++;
						return true;	
					}
				} finally {
			...
			if (!_memory) {
				...
				this.negValue = _buffer.getDouble();
				this.posValue = _buffer.getDouble();
				this.time = _buffer.getLong();
				this.negSwingProjection = _buffer.getDouble();
				this.posSwingProjection = _buffer.getDouble();
				_row ++;
			}
			return true;
		}
		...
		*/
		@Override
		public TriggerExpectedValueMDB getMDB() {
			return TriggerExpectedValueMDB.this;
		}
	}
		...
		public final class RandomCursor implements IRandomCursor<Record> {
			...
			RandomCursor() throws IOException {
				...
				if (!_memory) {
					...
					_channel = _raf.getChannel();
					_buffer = ByteBuffer.allocate(40);
			...
			public synchronized void seek(long position) throws IOException {
				...
				_row = position;
				_buffer.rewind();
				_channel.read(_buffer, position * 40);
				_buffer.rewind();
				this.negValue = _buffer.getDouble();
				this.posValue = _buffer.getDouble();
				this.time = _buffer.getLong();
	...
	*/
	...
	public static void writeBuffer(Cursor obj, ByteBuffer buffer) {
		buffer.putDouble(obj.negValue); 
		buffer.putDouble(obj.posValue); 
		buffer.putLong(obj.time); 
		buffer.putDouble(obj.negSwingProjection); 
		buffer.putDouble(obj.posSwingProjection); 
	}
	 ...
	 * @param val_negValue The value for column negValue.
	 * @param val_posValue The value for column posValue.
	 * @param val_time The value for column time.
	 * @param val_negSwingProjection The value for column negSwingProjection.
	 * @param val_posSwingProjection The value for column posSwingProjection.
	* @throws IOException If there is any I/O error.
	 */
	public void replace(long index 
							...
							, double val_posValue
							, long val_time
							, double val_negSwingProjection
							, double val_posSwingProjection
			) throws IOException {
		...
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					...
					if (pos < _rbufSize) {
						Record r = _rbuf[pos];
						r.negValue = val_negValue;
						r.posValue = val_posValue;
						r.time = val_time;
						r.negSwingProjection = val_negSwingProjection;
						r.posSwingProjection = val_posSwingProjection;
					}
					return;
				} 				
			}
			...
			_replaceBuffer.putDouble(val_negValue);
			_replaceBuffer.putDouble(val_posValue);
			_replaceBuffer.putLong(val_time);
			_replaceBuffer.putDouble(val_negSwingProjection);
			_replaceBuffer.putDouble(val_posSwingProjection);
			_replaceBuffer.rewind();
			appender();
			_appender._channel.write(_replaceBuffer, index * 40);
		} finally {
	 ...
	 * Also you can use {@link #replace(long , double, double, long, double, double)}.
	 *
	 * @param index The index to update.
	 * @param record Contains the data to set.
	 * @see #replace(long , double, double, long, double, double)
	 * @throws IOException If there is any I/O error.
	 */
	public void replace(long index, Record record) throws IOException {
		replace(index 
					...
					, record.posValue		
					, record.time		
					, record.negSwingProjection		
					, record.posSwingProjection		
				);			
	}

	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "negValue" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , double, double, long, double, double)},
	 ...
	 * @param value The new value to set to the column "negValue".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_negValue(long index, double value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].negValue = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_negValue.rewind();
			_replaceBuffer_negValue.putDouble(value);
			_replaceBuffer_negValue.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_negValue, index * 40 + 0);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "posValue" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , double, double, long, double, double)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "posValue".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_posValue(long index, double value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].posValue = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_posValue.rewind();
			_replaceBuffer_posValue.putDouble(value);
			_replaceBuffer_posValue.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_posValue, index * 40 + 8);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	...
	public void replace_time(long index, long value) throws IOException {
		...
		try {
			...
			_replaceBuffer_time.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_time, index * 40 + 16);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "negSwingProjection" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , double, double, long, double, double)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "negSwingProjection".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_negSwingProjection(long index, double value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].negSwingProjection = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_negSwingProjection.rewind();
			_replaceBuffer_negSwingProjection.putDouble(value);
			_replaceBuffer_negSwingProjection.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_negSwingProjection, index * 40 + 24);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "posSwingProjection" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , double, double, long, double, double)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "posSwingProjection".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_posSwingProjection(long index, double value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].posSwingProjection = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_posSwingProjection.rewind();
			_replaceBuffer_posSwingProjection.putDouble(value);
			_replaceBuffer_posSwingProjection.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_posSwingProjection, index * 40 + 32);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	...
	*/
	...
	public long fsize() throws IOException {		
		return _memory? 0 : _file.length() / 40;
	}
