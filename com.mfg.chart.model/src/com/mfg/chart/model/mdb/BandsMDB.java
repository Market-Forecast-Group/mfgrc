 ...
 * long new_val_topRaw = ...;
 * long new_val_centerRaw = ...;
 * long new_val_bottomRaw = ...;
 *
 * mdb.replace(index 
 * 		, new_val_time
 * 		, new_val_topPrice
 ...
 * <li>{@link BandsMDB#replace_topRaw(long, long)}: To replace the topRaw value.</li>
 * <li>{@link BandsMDB#replace_centerRaw(long, long)}: To replace the centerRaw value.</li>
 * <li>{@link BandsMDB#replace_bottomRaw(long, long)}: To replace the bottomRaw value.</li>
 * </ul>
 *
 * <h3>List API</h3>
 * TODO: Documentation is comming
 *
 * @see BaseChartMDBSession#connectTo_BandsMDB(String)
 */
...
/* BEGIN USER MDB */
	...
	*/	
	 ...
	 */
	public BandsMDB(BaseChartMDBSession session, File file, int bufferSize, SessionMode mode) throws IOException {
		...
		*/
		...
		public Object[] toArray() {
			return new Object[] {
							...
							Long.valueOf(topRaw),
							Long.valueOf(centerRaw),
							Long.valueOf(bottomRaw),
			 			};
		}
...
/**
		...
		public void append() throws IOException {
			if (_basic) {
				try {
					...
					_buf.putLong(this.topRaw);
					_buf.putLong(this.centerRaw);
					_buf.putLong(this.bottomRaw);
		...
		*/
		public void flush() throws IOException {	
			...
			for(int j = 0; j < _rbufSize; j++) {						
				...
				_buf.putLong(r.topRaw);
				_buf.putLong(r.centerRaw);
				_buf.putLong(r.bottomRaw);
			}
			_buf.rewind();
		...
		*/
		...
		public synchronized boolean next() throws IOException {
			...
			if (!_memory) {
				...
				this.topRaw = _buffer.getLong();
				this.centerRaw = _buffer.getLong();
				this.bottomRaw = _buffer.getLong();
				_row ++;
			}
			return true;
		}
	...
	*/
	...
	public static void writeBuffer(Cursor obj, ByteBuffer buffer) {
		...
		buffer.putLong(obj.topRaw); 
		buffer.putLong(obj.centerRaw); 
		buffer.putLong(obj.bottomRaw); 
	}
	...
	* @throws IOException If there is any I/O error.
	...
	public void replace(long index 
		...
		try {
			...
			_replaceBuffer.putLong(val_topRaw);
			_replaceBuffer.putLong(val_centerRaw);
			_replaceBuffer.putLong(val_bottomRaw);
			_replaceBuffer.rewind();
			appender();
	 ...
	 * Also you can use {@link #replace(long , long, double, double, double, long, long, long, long)}.
	 *
	 * @param index The index to update.
	 * @param record Contains the data to set.
	 * @see #replace(long , long, double, double, double, long, long, long, long)
	 * @throws IOException If there is any I/O error.
	 */
	 ...
	 * update many fields of the record then use {@link #replace(long , long, double, double, double, long, long, long, long)},
	 ...
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_topRaw(long index, long value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
			...
			_replaceBuffer_topRaw.putLong(value);
			_replaceBuffer_topRaw.rewind();
			appender();
	 ...
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_centerRaw(long index, long value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
			...
			_replaceBuffer_centerRaw.putLong(value);
			_replaceBuffer_centerRaw.rewind();
			appender();
	 ...
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_bottomRaw(long index, long value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
			...
			_replaceBuffer_bottomRaw.putLong(value);
			_replaceBuffer_bottomRaw.rewind();
			appender();
