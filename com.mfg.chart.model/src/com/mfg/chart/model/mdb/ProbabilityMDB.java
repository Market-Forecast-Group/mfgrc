...
/**
 * <p>
 * This class provides the API to manipulate Probability files. 
 * Here you will find the methods to modify and query the Probability files. 
 * </p>
 * <p>
 ...
 *	</tr>
 * </table>
 * <h3>ProbabilityMDB API</h3>
 * <p>
 * Now let's see the operations you can perform using this class on Probability files:
 * </p>
 *
 ...
 * ProbabilityMDB mdb = session.connectTo_ProbabilityMDB("probability.mdb");
 * 
 * // request the appender.
 * ProbabilityMDB.Appender app = mdb.appender(); 
 *
 ...
 * Probability.Cursor cursor = mdb.cursor(start, stop);
 *
 ...
 * ProbabilityMDB.RandomCursor cursor = mdb.randomCursor();
 *
 ...
 * // the new values 										
 * long new_val_time = ...;
 * double new_val_posPrice = ...;
 * double new_val_negPrice = ...;
 ...
 * </p>
 * <ul>
 * <li>{@link ProbabilityMDB#replace_time(long, long)}: To replace the time value.</li>
 * <li>{@link ProbabilityMDB#replace_posPrice(long, double)}: To replace the posPrice value.</li>
 * <li>{@link ProbabilityMDB#replace_negPrice(long, double)}: To replace the negPrice value.</li>
 * <li>{@link ProbabilityMDB#replace_posTradeDirection(long, boolean)}: To replace the posTradeDirection value.</li>
 * </ul>
 *
 * <h3>List API</h3>
 * TODO: Documentation is comming
 *
 * @see BaseChartMDBSession#connectTo_ProbabilityMDB(String)
 */

public final class ProbabilityMDB
/* BEGIN MDB EXTENDS */
		extends MDB<ProbabilityMDB.Record>
/* END MDB EXTENDS */
{
...
/* END USER MDB */
	/**
	 * Probability's meta-data: column names.
	 */
	...
	/**
	 * Probability's meta-data: column Java types.
	 */
	...
	/**
	 * Probability's meta-data: column Java types size (in bytes).
	 */
	...
	/**
	 * Probability's meta-data: virtual column flags.
	 */
	...
	/**
	 * Probability's meta-data: column byte-offset.
	 */
	...
	/**
	 * Probability's meta-data: size of the record, in bytes.
	 */
	...
	*/	
	...
	/**
	* posPrice's meta-data: index in a record.
	*/	
	public static final int COLUMN_POSPRICE = 1;
	/**
	* negPrice's meta-data: index in a record.
	*/	
	public static final int COLUMN_NEGPRICE = 2;
	/**
	* posTradeDirection's meta-data: index in a record.
	*/	
	public static final int COLUMN_POSTRADEDIRECTION = 3;

	/**
	 * Probability's meta-data: UUID used in schemas.
	 */
	public static final String TABLE_ID = "fe259a4b-572f-4564-a992-4b0f40255024";
	
	/**
	 * Probability's meta-data: signature used to check schema changes.
	 */ 
	public static final String TABLE_SIGNATURE = "7a7948df-ee59-4292-aea7-65748a0e14f5 LONG; da0a6f2e-bbdf-4ca6-919c-905111986e92 DOUBLE; d832d140-96d4-49af-acd4-71ec931ec8c0 DOUBLE; 1fa03292-ea83-4690-a149-ff746337d2d5 BOOLEAN; ";


	private Appender _appender;
	private ByteBuffer _replaceBuffer; 
	private ByteBuffer _replaceBuffer_time;
	private ByteBuffer _replaceBuffer_posPrice;
	private ByteBuffer _replaceBuffer_negPrice;
	 ...
	 */
	public ProbabilityMDB(BaseChartMDBSession session, File file, int bufferSize, SessionMode mode) throws IOException {
		...
		if (!_memory) {		
			...
			_replaceBuffer_posPrice = ByteBuffer.allocate(8);
			_replaceBuffer_negPrice = ByteBuffer.allocate(8);
			_replaceBuffer_posTradeDirection = ByteBuffer.allocate(1);
		}
	}	

	/**
	* Probability record structure.
	*/
...
/* END RECORD EXTENDS */	{
		...
		*/
		public long time; /* 0 */
		/**
		* Represents the posPrice column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of posPrice</caption>
		* <tr><td>Column</td><td>posPrice</td></tr>
		* <tr><td>Type</td><td>DOUBLE</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public double posPrice; /* 1 */
		/**
		* Represents the negPrice column.
		* <h2>Definition</h2>
		* <table border=1>
		* <caption>Definition of negPrice</caption>
		* <tr><td>Column</td><td>negPrice</td></tr>
		* <tr><td>Type</td><td>DOUBLE</td></tr>
		* <tr><td>Order</td><td>NONE</td></tr>
		* <tr><td>Virtual</td><td>No</td></tr>
		* </table>
		*/
		public double negPrice; /* 2 */
		/**
		...
		* </table>
		*/
		public boolean posTradeDirection; /* 3 */
		...
		*/
		...
		public Object[] toArray() {
			return new Object[] {
							Long.valueOf(time),
							Double.valueOf(posPrice),
							Double.valueOf(negPrice),
		...
		*/
		...
		public Object get(int columnIndex) {
			switch(columnIndex) {
				case 0: return time;
				case 1: return posPrice;
				case 2: return negPrice;
				case 3: return posTradeDirection;
		...
		*/ 
		public void update(Record record) {
			...
			this.posPrice = record.posPrice;
			this.negPrice = record.negPrice;
...
/**
	...
	* while( ... ) {
	* 	ap.time = getTime();	
	* 	ap.posPrice = getPosPrice();	
	* 	ap.negPrice = getNegPrice();	
		...
		*/
		...
		public void append() throws IOException {
			if (_basic) {
				try {
					...
					_buf.putDouble(this.posPrice);
					_buf.putDouble(this.negPrice);
				...
				try {
					...
					r.time = this.time;
					r.posPrice = this.posPrice;
					r.negPrice = this.negPrice;
		...
		public void append(Record record) throws IOException {	
			...
			try {										
				...
				Record r = _rbuf[_rbufSize];
				r.time = record.time;
				r.posPrice = record.posPrice;
				r.negPrice = record.negPrice;
		...
		*/
		public void flush() throws IOException {	
			...
			for(int j = 0; j < _rbufSize; j++) {						
				Record r = _rbuf[j];
				_buf.putLong(r.time);
				_buf.putDouble(r.posPrice);
				_buf.putDouble(r.negPrice);
		...
		*/
		...
		public synchronized boolean next() throws IOException {
			...
			if (!_basic) {
				...
				try {
					if (_rbufSize > 0 && _row >= _rbufPos) {
						...
						this.time = r.time;
						this.posPrice = r.posPrice;
						this.negPrice = r.negPrice;
			...
			if (!_memory) {
				...
				this.posPrice = _buffer.getDouble();
				this.negPrice = _buffer.getDouble();
	...
	*/
	...
	public static void writeBuffer(Cursor obj, ByteBuffer buffer) {
		...
		buffer.putDouble(obj.posPrice); 
		buffer.putDouble(obj.negPrice); 
	 ...
	 * @param val_posPrice The value for column posPrice.
	 * @param val_negPrice The value for column negPrice.
	...
	* @throws IOException If there is any I/O error.
	...
	public void replace(long index 
		...
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					...
					if (pos < _rbufSize) {
						Record r = _rbuf[pos];
						r.time = val_time;
						r.posPrice = val_posPrice;
						r.negPrice = val_negPrice;
			...
			_replaceBuffer.putDouble(val_posPrice);
			_replaceBuffer.putDouble(val_negPrice);
	 ...
	 * Also you can use {@link #replace(long , long, double, double, boolean)}.
	 *
	 * @param index The index to update.
	 * @param record Contains the data to set.
	 * @see #replace(long , long, double, double, boolean)
	 * @throws IOException If there is any I/O error.
	 */
	 ...
	 * update many fields of the record then use {@link #replace(long , long, double, double, boolean)},
	...
	public void replace_time(long index, long value) throws IOException {
		...
		try {
			...
			_replaceBuffer_time.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_time, index * 25 + 0);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "posPrice" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , long, double, double, boolean)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "posPrice".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_posPrice(long index, double value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].posPrice = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_posPrice.rewind();
			_replaceBuffer_posPrice.putDouble(value);
			_replaceBuffer_posPrice.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_posPrice, index * 25 + 8);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	/**
	 * Update the record at the given <code>index</code>, 
	 * but only the column "negPrice" is updated. If you like to 
	 * update many fields of the record then use {@link #replace(long , long, double, double, boolean)},
	 * but if you want to update just one column, use this method because it is faster.
	 * 
	 * @param index The index of the record to update.
	 * @param value The new value to set to the column "negPrice".
	 *
	 * @throws IOException If there is any I/O error.
	 */
	public void replace_negPrice(long index, double value) throws IOException {
		if (!_basic) _writeLock.lock();
		try {
			if (!_basic) {
				if (index >= _rbufPos) {
					int pos = (int) (index - _rbufPos);
					if (pos < _rbufSize) {
						_rbuf[pos].negPrice = value;
					}
					return;
				} 				
			}
		
			_replaceBuffer_negPrice.rewind();
			_replaceBuffer_negPrice.putDouble(value);
			_replaceBuffer_negPrice.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_negPrice, index * 25 + 16);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
	...
	public void replace_posTradeDirection(long index, boolean value) throws IOException {
		...
		try {
			...
			_replaceBuffer_posTradeDirection.rewind();
			appender();
			_appender._channel.write(_replaceBuffer_posTradeDirection, index * 25 + 24);
		} finally {
			if (!_basic) _writeLock.unlock();
		}		
	}
