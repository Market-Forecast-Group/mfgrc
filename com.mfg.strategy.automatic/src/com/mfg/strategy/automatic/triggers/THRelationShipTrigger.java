...
public class THRelationShipTrigger extends ValueScaleSpecificTrigger {
	...
	protected boolean check_THRelationship() {
		if (isEnabled()) {
			...
			if (!getRelationshipType().check(fWidget.isSwingDown(fWidgetScale), TH0, THm1))
				return false;
	...
	public static double getTHRatio(TRIGGER_TYPE aValueType, IIndicator widget, int widgetScale, int step) {
	...
	/**
	 * gets the type of the TH relationship (C, NC, None see {@link RelationshipType})
	 * 
	...
	/**
	 * sets the type of the TH relationship (C, NC, None see {@link RelationshipType})
	 * 
	...
	/**
	 * gets which thresholds ratios are we checking (TH<sub>0</sub>, TH<sub>-1</sub> or Both) to in an specific range.
	 * 
	...
	/**
	 * sets which thresholds ratios are we checking (TH<sub>0</sub>, TH<sub>-1</sub> or Both) to in an specific range.
	 * 
	 * @param aTHType
	 ...
	 */
	@Override
	public THRelationShipTrigger clone() {
		return (THRelationShipTrigger) super.clone();
	}
	...
	public int hashCode() {
		...
		result = prime * result + ((fRelationshipType == null) ? 0 : fRelationshipType.hashCode());
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		THRelationShipTrigger other = (THRelationShipTrigger) obj;
		...
		if (fValueType != other.fValueType)
			return false;
		if (Double.doubleToLongBits(thRatioL) != Double.doubleToLongBits(other.thRatioL))
			return false;
		if (Double.doubleToLongBits(thRatioU) != Double.doubleToLongBits(other.thRatioU))
			return false;
		return true;
	}
	...
	public String getHtmlBody(HtmlUtils aUtil) {
		...
		res = ("{THRelation=" + getRelationshipType()) + (", TH=" + getTHType()) + (", Range=[" + aUtil.color("" + thRatioL, Color.blue) + ", " + aUtil.color("" + ((thRatioU == 1) ? "Inf" : Double.valueOf(thRatioU)), Color.red) + ")") + "}";
		return res + "}";
	}
