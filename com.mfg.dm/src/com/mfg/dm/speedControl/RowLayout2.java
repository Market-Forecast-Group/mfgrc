 ...
 * <code>Composite</code> by placing them either in horizontal rows or vertical
 * columns within the parent <code>Composite</code>.
 * <p>
 ...
 * </p>
 * <p>
 * The following example code creates a <code>RowLayout</code>, sets all of its
 * fields to non-default values, and then sets it into a <code>Shell</code>.
 * 
 * <pre>
 ...
 * 
 * @see RowData
 * @see <a href="http://www.eclipse.org/swt/snippets/#rowlayout">RowLayout
 *      snippets</a>
 ...
 *      information</a>
 */
@SuppressWarnings({"null","hiding", "static-method"})
public final class RowLayout2 extends Layout {
	...
	protected Point computeSize(Composite composite, int wHint, int hHint,
			boolean flushCache) {
		out.println("computeSize(Composite composite, int wHint, int hHint, boolean flushCache)");
		Point extent;
	...
	Point computeSize(Control control, boolean flushCache) {
		out.println("computeSize(Control control, boolean flushCache)");
	...
	protected void layout(Composite composite, boolean flushCache) {
		...
		if (type == SWT.HORIZONTAL) {
			layoutHorizontal(composite, true, wrap, clientArea.width,
					flushCache);
		} else {
	...
	Point layoutHorizontal(Composite composite, boolean move, boolean wrap,
			int width, boolean flushCache) {
		...
		int maxX = 0, x = marginLeft + marginWidth, y = marginTop
				+ marginHeight;
		for (int i = 0; i < count; i++) {
			Control child = children[i];
			if (pack) {
			...
			if (move) {
				...
				if (justify || fill || center) {
					bounds[i] = new Rectangle(childX, childY, childWidth,
							childHeight);
				} else {
			...
			for (int i = 0; i < count; i++) {
				if (!wrapped) {
					...
					if (fill) {
						...
						if (center) {
							bounds[i].y += Math.max(0,
									(maxHeight - bounds[i].height) / 2);
						}
					}
				}
	...
	Point layoutVertical(Composite composite, boolean move, boolean wrap,
		...
		int maxY = 0, x = marginLeft + marginWidth, y = marginTop
				+ marginHeight;
		for (int i = 0; i < count; i++) {
			Control child = children[i];
			if (pack) {
			...
			for (int i = 0; i < count; i++) {
				if (!wrapped) {
					...
					if (fill) {
						...
						if (center) {
							bounds[i].x += Math.max(0,
									(maxWidth - bounds[i].width) / 2);
						}
					}
	...
	public String toString() {
		...
		string += "type="
				+ ((type != SWT.HORIZONTAL) ? "SWT.VERTICAL" : "SWT.HORIZONTAL")
				+ " ";
		if (marginWidth != 0)
